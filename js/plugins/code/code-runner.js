...
class CodeRunner extends LitElement {
    ...
    handleLanguageChange(e) {
        ...
        if (this.language === 'python' && !this.pyodide) {
            this.initializePyodide();
        }
        this.requestUpdate();
        if (window.wisk?.editor?.justUpdates) {
    ...
    async setValue(identifier, value) {
        ...
        const shouldExecute = value.code !== this.code || value.language !== this.language;
        ...
        if (value.importedPackages !== undefined) {
            ...
            // Reimport packages if python is selected
            if (this.language === 'python' && this.pyodide) {
        ...
        // Execute code if there was a change in code or language
        if (shouldExecute && this.code) {
            // Small delay to ensure Pyodide is fully initialized if needed
    ...
    handleTabKey(e) {
        if (e.key === 'Tab') {
            ...
            // Call justUpdates after tab insertion
            if (window.wisk?.editor?.justUpdates) {
                window.wisk.editor.justUpdates(this.id);
            }
        }
    ...
    render() {
        return html`
            <div class="op">
                <div class="controls">
                    <select @change=${this.handleLanguageChange} .value=${this.language}>
                        <option value="javascript">JavaScript</option>
                        <option value="python">Python</option>
                    </select>
                    ${this.language === 'python' ? html`
                ...
                <textarea 
                    ...
                    .value=${this.code}
                    spellcheck="false"
                    placeholder=${this.language === 'python' ? "print('Hello, World!')" : "console.log('Hello, World!')"}
                ></textarea>
