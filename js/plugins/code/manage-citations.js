    ...
    convertBibtexToReference(bibtexEntry) {
        return {
            ...
            authors: bibtexEntry.author ? 
                    bibtexEntry.author.split(' and ').map(author => author.trim()) : [],
...
formatDate(dateString) {
    ...
    formatAuthors(authors, format) {
        ...
        return authors.map(formatAuthor).filter(a => a).join(', ');
    }
        ...
        switch (this.selectedFormat) {
            ...
            case 'vancouver':
                return `${reference.authors.map((_, i) => i + 1).join('')}. ${authors}. ${reference.title}. ${
                    reference.journal_conference || ''} ${date.year || ''};${
            ...
            case 'chicago':
                return `${authors}. "${reference.title}." ${
                    ...
                    reference.issue ? `no. ${reference.issue} ` : ''
                }(${date.year || 'n.d.'}): ${reference.pages || 'n.p.'
                }${reference.doi ? `. DOI:${reference.doi}` : ''}.`;
            ...
            case 'turabian':
                return `${authors}. "${reference.title}." ${
                    reference.journal_conference ? `${reference.journal_conference} ` : ''
                }${reference.volume || ''}${reference.issue ? `, no. ${reference.issue}` : ''
                } (${date.year || 'n.d.'}): ${reference.pages || ''
                }${reference.doi ? `. DOI: ${reference.doi}` : ''}.`;
...
addReference(event) {
    ...
    const publishDate = form.publish_date.value ? 
        ...
        this.references = this.references.map(ref => 
            ref.id === this.editingId ? updatedReference : ref
        );
        this.editingId = null;
    }
        ...
        return html`
            <form class="form" @submit=${reference.id ? this.updateReference : this.addReference}>
                <div class="field-group">
                    ...
                    <input class="input" name="authors" placeholder="Authors (comma-separated) *" value=${reference.authors ? reference.authors.join(', ') : ''} required>
                ...
                <div class="field-group">
                    ...
                    <input class="input" name="journal_conference" placeholder="Journal/Conference Name" value=${reference.journal_conference || ''}>
            ...
            <div class="container">
                <div class="header-actions">
                    ...
                    <div style="display: flex; gap: var(--gap-2);">
                        ...
                        <button class="button" @click=${() => this.showBibtexModal = true}>
                            Paste BibTeX
                        </button>
                    </div>
                    <button class="button button-primary" @click=${this.toggleCreateForm}>
                        ${this.isCreating ? 'Cancel' : 'Add New Citation'}
                    </button>
